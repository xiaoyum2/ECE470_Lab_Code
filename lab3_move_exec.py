...
gridzero_inRobotWorld_y = -0.0375
...
class particleFilter:
    ...
    def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
        ...
        new_particles = np.random.uniform(0,1,(self.numParticles,2))
        for i in range(self.numParticles):
            new_particles[i] = np.multiply(new_particles[i],self.curMax)
        self.particles = new_particles
        self.weights = np.ones(self.numParticles)*1./self.numParticles
    ...
    def Measurement_Model(self, x, y):
        ...
        ####################################################################
        # Students finish the code below
        loc_reading = self.model.readingSensor(x,y)
        set_wt = []
        old_particles = self.particles
        for i in range(self.numParticles):
            tempreading = self.model.readingMap(old_particles[i])
            tempdis = np.sqrt((loc_reading[0]-tempreading[0])**2+(loc_reading[1]-tempreading[1])**2+(loc_reading[2]-tempreading[2])**2+(loc_reading[3]-tempreading[3])**2)
            tempwt = np.exp(-tempdis**2/(2*self.std))
            set_wt.append(tempwt)
        sum_weight = sum(set_wt)
        if sum_weight==0:
            norm_wt = [1/self.numParticles for i in range(self.numParticles)]
        else:
            norm_wt = [float(i)/sum_weight for i in set_wt]
    ...
    def calcPosition(self):
        ...
        temp_par = self.particles
        temp_weight = self.weights
        x = 0
        y = 0
        
        for i in range(self.numParticles):
            x = x+temp_par[i][0]*temp_weight[i]
            y = y+temp_par[i][1]*temp_weight[i]
        # print(x)
        # print(temp_par[i][0])
        # print(type(self.curMin[0]))
        # if x < 0:
        ...
        x = min(x,self.curMax[0])
        x = max(x,self.curMin[0])
        y = min(y,self.curMax[1])
        y = max(y,self.curMin[1])
        
        # if y < self.curMin[1]:
        #     estimatePosition[1] = self.curMin[1]
        # elif x > self.curMax[1]:
